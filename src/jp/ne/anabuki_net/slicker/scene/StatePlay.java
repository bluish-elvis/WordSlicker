...
public class StatePlay extends BaseGameState{
	...
	String str="";
	String quest="The quick brown fox jumped over the lazy dogs";
	int pos=0;
	...
	@Override public void renderImpl(GameContainer container, StateBasedGame game, Graphics g) throws SlickException{
		...
		if(pos<quest.length())
			g.drawString(quest.substring(pos), 32, 32);
	}
	...
	@Override public void updateImpl(GameContainer gc, StateBasedGame game, int delta) throws SlickException{
		...
		/* when finished : MySQLConnector ScoreUpdate = new MySQLConnector();
	...
	@Override public void keyPressed(int key, char c){
		...
		str=""+c;
		if(Character.toLowerCase(quest.charAt(pos))==c)
			do{
			pos++;
			if(pos>=quest.length()){ //when subject complete
			break;
			}
			}while(!Character.isLetter(quest.charAt(pos)));
